...
def evaluate_quick(model, train_seqs, eval_items, num_items,
    ...
    """Sampled evaluation for validation. 99 neg + 1 pos per user."""
    model.eval()
...
def train(args):
    ...
    log.info(f"training SASRec on {args.dataset} (device={device})")
    ...
    best_ndcg = -1.0
    best_ckpt = CKPT_DIR / f'sasrec_{args.dataset}_best.pt'
