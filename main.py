...
def train(config, verbose=True, rank=0):
    ...
    item_feature_path = config.get('item_feature_path', None)
    item_features_matrix = None
    if use_features and item_feature_path is not None:
        feature_full_path = os.path.join(dataset_path, item_feature_path)
        if os.path.exists(feature_full_path):
            item_features_matrix = np.load(feature_full_path)
            log(f"Loaded item features from {feature_full_path}", accelerator, logger)
        else:
            log(f"Warning: Feature file {feature_full_path} not found. Training without features.", accelerator, logger)
    ...
    model_rec = Model(config=config, model=t5, 
                  n_items=num_items,
    ...
    train_dataset = SequentialSplitDataset(config=config, n_items=num_items, inter_seq=train_seq, item_features_matrix=item_features_matrix)
    valid_dataset = SequentialSplitDataset(config=config, n_items=num_items, inter_seq=valid_seq, item_features_matrix=item_features_matrix)
    test_dataset = SequentialSplitDataset(config=config, n_items=num_items, inter_seq=test_seq, item_features_matrix=item_features_matrix)
