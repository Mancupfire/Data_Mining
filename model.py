...
class Model(nn.Module, GenerationMixin):
    def __init__(self, config, model, n_items, code_length=1, code_number=256):
        ...
        self.num_features = config.get('num_features', 0)
        if self.use_features and self.num_features > 0:
            feature_layers = [self.num_features, self.hidden_size, self.hidden_size]
            self.feature_adapter = MLPLayers(layers=feature_layers, dropout=config.get('dropout_prob', 0.1))
    ...
    def prepare_inputs_for_generation(self, input_ids, attention_mask=None, encoder_outputs=None, item_features=None, **kwargs):
        return {"decoder_input_ids": input_ids, "encoder_outputs": encoder_outputs, "attention_mask": attention_mask, "item_features": item_features}
    ...
    def get_input_embeddings(self, input_ids, attention_mask, item_features=None):
        ...
        # --- Inject Feature Embeddings ---
        if self.use_features and item_features is not None:
            feature_embeds = self.feature_adapter(item_features)
            # FIX: Lặp lại feature 4 lần (code_length) để khớp với 200 token của chuỗi
            feature_embeds = feature_embeds.repeat_interleave(self.code_length, dim=1)
            inputs_embeds = inputs_embeds + feature_embeds

        return inputs_embeds
    
    def forward(self, input_ids=None, inputs_embeds=None, attention_mask=None, labels=None, decoder_input_ids=None,
                decoder_inputs_embeds=None, encoder_outputs=None, item_features=None, **kwargs):
        
        if input_ids is not None:
            inputs_embeds = self.get_input_embeddings(input_ids, attention_mask, item_features)
    ...
    def generate(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, n_return_sequences: int = 1,
                 prefix_allowed_tokens_fn=None, item_features=None) -> torch.Tensor:
    ...
    def my_beam_search(self, input_ids, attention_mask, max_length=6, num_beams=1, num_return_sequences=1, return_score=False, item_features=None):
        ...
        if item_features is not None:
            item_features = item_features.repeat_interleave(num_beams, dim=0)
        ...
        while decoder_input_ids.shape[1] < max_length:
            ...
            decoder_input_ids, beam_scores = self.beam_search_step(
                outputs.logits, decoder_input_ids, beam_scores, beam_idx_offset, batch_size, num_beams
            )
