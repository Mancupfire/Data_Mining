...
_Date: 2026-06-05. Repo: `/mnt/disk1/backup_user/minh.ntn/ETEGRec`. Single GPU (CUDA 7)._
...
_Full section: `outputs/report_sections/kmeans_codebook_visualization.md`; figures in `outputs/figures/`._
...
| Game (Full SEA-Rec) | PENDING | — | — | — | run after Scientific ablations |

Full test row: R@1 0.0075 · R@5 0.0265 · R@10 0.0419 · N@5 0.0169 · N@10 0.0219.
...
Each variant removes one component; sweep runs sequentially on GPU 7 with a **shared reduced
budget** (labelled `run_type=short`, comparable to each other — NOT to the full-schedule headline).

| Variant | KL | Dec-CL | Features | run_type | Status |
|---|:--:|:--:|:--:|---|---|
| A. Full SEA-Rec | ✓ | ✓ | ✓ | full | **DONE** (headline, R@10 0.0419 / N@10 0.0219) |
| A. Full SEA-Rec (short) | ✓ | ✓ | ✓ | short | RUNNING |
| B. w/o KL | ✗ | ✓ | ✓ | short | PENDING (queued) |
...
| G. k-means vs random init | — | — | — | — | planned, not run (needs RQ-VAE re-pretrain) |

---

## 8. Next steps / pending runs

1. **Finish the Scientific ablation sweep** (running): `bash scripts/run_minh_ablation_scientific.sh`
   (B→C→D→E sequential). Then `python scripts/parse_training_results.py` to fill PENDING rows.
2. **Game full run** (after Scientific ablations free the GPU):
   `bash run_game.sh` (no feature adapter — game has no `item_features.npy`).
3. **Game codebook after-training** once its checkpoint exists:
   `python scripts/analyze_codebook_utilization.py --config ./config/game.yaml --tag after --rqvae_ckpt <best>.pt.rqvae`
   then `python scripts/plot_codebook_utilization.py --dataset game`.
4. (Optional) **Variant G** k-means vs random init: `cd RQVAE && bash run_pretrain.sh` (with
   `--kmeans_init True`), repoint `rqvae_path`, re-run.
